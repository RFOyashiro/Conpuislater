...
n_exp *comp(void) {
	...
	n_exp *$compb = NULL;
	openXML(__func__);
	if (est_premier(_modulo_, currentUnit)) {
		$m = m();
		$compb = compb($m);
		$comp = $compb;
		closeXML(__func__);
...
n_exp *mb(n_exp *herit) {
	...
	n_exp *$mb2 = NULL;
	openXML(__func__);
	if (checkToken(MODULO)) {

		$e = e();
		$mb2 = mb($e);
		$mb = cree_n_exp_op(modulo, herit, $mb2);
		closeXML(__func__);
		return $mb;
	}
	if (est_suivant(_moduloBis_, currentUnit)) {
...
n_exp *eb(n_exp *herit) {
	...
	if (checkToken(MOINS)) {
		...
		$t = t();
		$eb = cree_n_exp_op(moins, herit, $t);
		$eb2 = eb($eb);
		closeXML(__func__);
		return $eb2;
	}
